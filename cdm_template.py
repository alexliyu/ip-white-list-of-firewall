...
def load(theme,template_file, debug=False):
  ...
  if not template:
	...
	try:
		mfile=memcache.get(abspath)
		if mfile is None:
			t = open(abspath).read()
			logging.info('add template %s to memcache',abspath)
			memcache.add(abspath,t,time=720000)
		else:
			t = mfile
			logging.info('load template %s from memcache',abspath)
		template = django.template.Template(t)
	finally:
